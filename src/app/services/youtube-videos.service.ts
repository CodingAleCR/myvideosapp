...
export class YoutubeVideosService extends VideosService {
  ...
  private key = "AIzaSyDlbNOX3nbkOknjXIPq1q-5eqYkBdvY1j0";
  ...
  findVideos(query: string): Promise<Video[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.http.get(`${this.urlBase}/search`, { params: params }).subscribe(
        (data: any) => {
        ...
        err => {
          console.log(
            `[YoutubeVideosService] findVideos(${query}) ERROR: ` +
              JSON.stringify(err)
          );
          reject(err);
        }
  ...
  findVideoById(id: string): Promise<Video> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.http.get(`${this.urlBase}/videos`, { params: params }).subscribe(
        (data: any) => {
        ...
        err => {
          console.log(
            `[YoutubeVideosService] findVideoById(${id}) ERROR: ` +
              JSON.stringify(err)
          );
          reject(err);
        }
  ...
  addVideo(video: Video): Promise<Video> {
    console.log(
      "[YoutubeVideosService] addVideo(" + JSON.stringify(video) + ")"
    );
    return new Promise((resolve, reject) => {
  ...
  updateVideo(video: Video): Promise<Video> {
    console.log(
      "[YoutubeVideosService] updateVideo(" + JSON.stringify(video) + ")"
    );
    return new Promise((resolve, reject) => {
      reject(new Error("Not supported operation"));
    });
