...
export class MemoryVideosService extends VideosService {
  ...
  addVideo(video: Video): Promise<Video> {
    console.log(
      "[MemoryVideosService] addVideo(" + JSON.stringify(video) + ")"
    );
  ...
  removeVideo(id: string): Promise<void> {
    ...
    if (index !== -1) {
      ...
      return new Promise((resolve, reject) =>
        reject(
          new Error(`Video with id ${id} not found`)
        )
      );
    }
  }
  ...
  updateVideo(video: Video): Promise<Video> {
    console.log(
      "[MemoryVideosService] updateVideo(" + JSON.stringify(video) + ")"
    );
    ...
    if (index !== -1) {
      ...
      return new Promise((resolve, reject) =>
        reject(
          new Error(`Video with id ${video.id} not found`)
        )
      );
    }
  }
